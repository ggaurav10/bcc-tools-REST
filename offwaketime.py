...
def offwaketimeUtil(duration):
  ...
  thread_context = ""
  if tgid is not None:
    thread_context = "PID %d" % tgid
    thread_filter = 'tgid == %d' % tgid
  elif pid is not None:
    thread_context = "TID %d" % pid
    thread_filter = 'pid == %d' % pid
  elif user_threads_only:
    thread_context = "user threads"
    thread_filter = '!(p->flags & PF_KTHREAD)'
  elif kernel_threads_only:
  ...
  if not folded:
    print("Tracing blocked time (us) by %s off-CPU and waker stack" %
        stack_context, end="", file=output)
    if duration < 99999999:
        print(" for %d secs." % duration, file=output)
    else:
        print("... Hit Ctrl-C to end.", file=output)
  ...
  for k, v in sorted(counts.items(), key=lambda counts: counts[1].value):
    ...
    if folded:
        ...
        print("    %-16s %s %s" % ("waker:", k.waker.decode(), k.t_pid), file=output)
        ...
        for addr in target_user_stack:
            print("    %s" % b.sym(addr, k.tgid), file=output)
        print("    %-16s %s %s" % ("target:", k.target.decode(), k.w_pid), file=output)
        print("        %d\n" % v.value, file=output)
  ...
  b.detach_kprobe(event="try_to_wake_up")
  return output.getvalue()

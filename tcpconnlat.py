...
def print_ipv4_event(cpu, data, size):
    ...
    print("%-6d %-12.12s %-2d %-16s %-16s %-5d %.2f" % (event.pid, event.task,
        ...
        float(event.delta_us) / 1000), file=output)
...
def detach(b):
	...
	b.detach_kprobe(event="tcp_v6_connect")
	b.detach_kprobe(event="tcp_rcv_state_process")

def tcpconnlatUtil(duration, pid=None):
	output.truncate(0)
	output.seek(0)
	b = init_bpf(pid)
	...
	print("%-6s %-12s %-2s %-16s %-16s %-5s %s" % ("PID", "COMM", "IP", "SADDR",
     	   "DADDR", "DPORT", "LAT(ms)"), file=output)

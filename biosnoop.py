...
def detach(b):
	...
	b.detach_kprobe(event="blk_start_request")
	b.detach_kprobe(event="blk_mq_start_request")
...
def print_event(cpu, data, size):
    ...
    print("%-14.9f %-14.14s %-6s %-7s %-2s %-9s %-7s %7.2f" % (
        ...
        event.len, float(event.delta) / 1000000), file=output)

    prev_ts = event.ts
    start_ts = 1

def biosnoopUtil(duration):
        output.truncate(0)
        output.seek(0)
	...
	print("%-14s %-14s %-6s %-7s %-2s %-9s %-7s %7s" % ("TIME(s)", "COMM", "PID",
		"DISK", "T", "SECTOR", "BYTES", "LAT(ms)"), file=output)

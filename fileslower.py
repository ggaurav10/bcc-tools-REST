...
def init_bpf(min_ms, tgid, all_files):
	# define BPF program
	bpf_text = """
...
def detach(b):
        b.detach_kprobe(event="__vfs_read")
        b.detach_kretprobe(event="__vfs_read")
        try:
                b.detach_kprobe(event="__vfs_write")
                b.detach_kretprobe(event="__vfs_write")
        except:
                # older kernels don't have __vfs_write so try vfs_write instead
                b.detach_kprobe(event="vfs_write")
                b.detach_kretprobe(event="vfs_write")
...
def print_event(cpu, data, size):
    ...
    print("%-8.3f %-14.14s %-6s %1s %-7s %7.2f %s" % (
        ...
        event.sz, ms, name), file=output)

def fileslowerUtil(duration, min_ms, pid, all_files):
	output.truncate(0)
	output.seek(0)

	b = init_bpf(min_ms, pid, all_files)

	# header
	print("Tracing sync read/writes slower than %d ms" % min_ms, file=output)
	print("%-8s %-14s %-6s %1s %-7s %7s %s" % ("TIME(s)", "COMM", "TID", "D",
		"BYTES", "LAT(ms)", "FILENAME"), file=output)

...
def print_ipv4_event(cpu, data, size):
    ...
    print("%-8s %-6d %-2d %-20s %1s> %-20s %s" % (
        ...
        tcpstate[event.state]), file=output)
...
def tcpretransUtil(duration, lossprobe=None):
	global output
	output.truncate(0)
	output.seek(0)
	...
	print("%-8s %-6s %-2s %-20s %1s> %-20s %-4s" % ("TIME", "PID", "IP",
	     "LADDR:LPORT", "T", "RADDR:RPORT", "STATE"), file=output)
	...
	if lossprobe:
		b.detach_kprobe(event="tcp_send_loss_probe")
